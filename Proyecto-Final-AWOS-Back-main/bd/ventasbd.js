...
var buscarUsuarios = require("../bd/usuariosbd").buscarUsuarios; // Importación corregida

async function validarDatos(venta) {
    var valido = false;
    if (
        venta.fechayhora != undefined &&
        venta.id_usuario != undefined &&
        venta.id_producto != undefined &&
...
async function validarDatosNuevos(venta) {
    ...
    if (
        ...
        if (await buscarUsuarios(venta.id_usuario)) { // Uso de la función corregida
